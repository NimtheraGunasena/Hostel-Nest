...
const Pickup = () => {
  ...
  const { selectedFoods, foodPrices } = location.state || { selectedFoods: {}, foodPrices: {} };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3000/api/pickup/addPickupDetails', {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        ...
        // Navigate to the food home page
        navigate('/foodhome');
        // Clear input fields
        setFormData({
          username: '',
  ...
  const calculateTotalAmount = () => {
    return Object.entries(selectedFoods).reduce((total, [foodId, quantity]) => {
      return total + (quantity * foodPrices[foodId]);
    }, 0);
  };
