...
const BreakfastMenu = () => {
  ...
  const fetchFoods = useCallback(async () => {
    try {
      const response = await fetch('http://localhost:3000/api/food/foodCategory/Breakfast');
  ...
  const handleIncrement = useCallback((food) => {
  ...
  const handleDecrement = useCallback((food) => {
    setSelectedFoods((prev) => {
      if (prev[food._id] > 0) {
        return {
          ...prev,
    ...
    <div className="min-h-screen bg-cover bg-center bg-no-repeat p-4" style={{ backgroundImage: "url('https://th.bing.com/th/id/R.c09cc81e4e1dbd86b9b01001ae8e719f?rik=iPTv8bpRV7ZD1Q&pid=ImgRaw&r=0')" }}>
      <div className="max-w-5xl mx-auto mt-20 mb-20 bg-slate-950 bg-opacity-90 rounded-lg shadow-md p-6">
        ...
        <div className="flex justify-center mt-6">
          <button onClick={handleSelect} className="bg-green-500 ml-96 w-44 text-black py-2 px-6 rounded-lg font-bold">
            Select
          </button>
        </div>
      </div>
